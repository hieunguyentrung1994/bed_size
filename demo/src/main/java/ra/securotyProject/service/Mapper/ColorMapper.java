...
public class ColorMapper implements IGenericMapper<Color, ColorRequest, ColorResponse> {
    ...
    public Color toEntity(ColorRequest colorRequest) {
        return Color.builder().nameColor(colorRequest.getColorName()).status(colorRequest.isStatus()).build();
    }

    @Override
    public ColorResponse toResponse(Color color) {
        return ColorResponse.builder().colorName(color.getNameColor()).status(color.isStatus()).id(color.getId()).build();
    }
}
